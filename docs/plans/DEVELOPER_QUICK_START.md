...
const camera = new THREE.OrthographicCamera(
  -48,
  48, // X bounds (bar width)
  36,
  -36, // Y bounds (bar depth)
  1,
  1000 // Near/far planes
);
...
const lights = {
  gaming: new THREE.PointLight(0x88aaff, 0.7, 12), // Blue-white, 70%, 12ft range
  central_bar: new THREE.PointLight(0xffaa44, 0.5, 10), // Amber, 50%, 10ft range
  card_tables: new THREE.PointLight(0xff4444, 0.4, 8), // Red, 40%, 8ft range
  firepit: new THREE.PointLight(0xff6622, 0.3, 6), // Orange, 30%, 6ft range (firelight)
};
...
// Stage (south side)
const stage = new THREE.Mesh(new THREE.BoxGeometry(10, 10, 8), secondFloorMaterial);
...
function updateSpatialAudio() {
  remoteUsers.forEach((user) => {
    ...
    if (
      localBooth &&
      localBooth.startsWith('booth_') &&
      remoteBooth &&
      remoteBooth.startsWith('booth_')
    ) {
      // Both in booths
      ...
      (localBooth && localBooth.startsWith('booth_')) ||
